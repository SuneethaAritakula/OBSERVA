...
public class ContinuityPage {
	...
	// 04/10/2019 - this method is only giving size of rows and cols - no text captured				
	/*public void tblFirstInput()
					
					{
		
		
		List<WebElement>headers = tblFirstInput.findElement(By.tagName("thead")).findElements(By.tagName("td"));
		...
		List<WebElement>rows = tblFirstInput.findElements(By.tagName("tr"));	
                   ...
                   /*List<WebElement>tablebodyrows = tblFirstInput.findElements(By.tagName("tbody"));	
						
						for(int j = 0;j<=rows.size();j++)
							
						{
							List<WebElement>tablebodycols = rows.get(j).findElements(By.tagName("td"));		
							
						for(int k = 0; k<tablebodycols.size(); k++)
						{
							String data = tablebodycols.get(k).getText();
...
List<WebElement>headerrows = tblFirstInput.findElements(By.tagName("thead"));	
	...
	{
		List<WebElement>cols = headerrows.get(i).findElements(By.tagName("tr"));
		//System.out.println(cols.size());
